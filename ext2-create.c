...
void write_block_bitmap(int fd) {
	...
	/* Set your bits here...  */
	set_bit(bitmap, SUPERBLOCK_BLOCKNO);
	set_bit(bitmap, BLOCK_GROUP_DESCRIPTOR_BLOCKNO);
	set_bit(bitmap, BLOCK_BITMAP_BLOCKNO);
	set_bit(bitmap, INODE_BITMAP_BLOCKNO);
	// Inode table
	for (u32 i = 0; i <= ROOT_DIR_BLOCKNO - INODE_BITMAP_BLOCKNO; i++) {
		set_bit(bitmap, INODE_TABLE_BLOCKNO + i);
	}
	set_bit(bitmap, ROOT_DIR_BLOCKNO);
	set_bit(bitmap, LOST_AND_FOUND_DIR_BLOCKNO);
	set_bit(bitmap, HELLO_WORLD_FILE_BLOCKNO);
	set_bit(bitmap, BLOCK_SIZE); // Last block 
	// Padding
	for (u32 block_num = BLOCK_SIZE + 1; block_num <= NUM_BLOCKS * 8; block_num++) {
		set_bit(bitmap, block_num);
	}

	/* Get to the right spot in img to write to.  */
	off_t off = lseek(fd, BLOCK_OFFSET(BLOCK_BITMAP_BLOCKNO), SEEK_SET);
	if (off == -1)
	{
		errno_exit("lseek");
	}
	/* Write your data in bitmap to img.  */
...
void write_inode_bitmap(int fd) {
	...
	/* Set your bits here...  */
	set_bit(bitmap, EXT2_ROOT_INO);
	set_bit(bitmap, LOST_AND_FOUND_INO);
	set_bit(bitmap, HELLO_WORLD_INO);
	set_bit(bitmap, HELLO_INO);
	for (u32 inode_num = 1; inode_num < EXT2_GOOD_OLD_FIRST_INO; inode_num++) {
		set_bit(bitmap, inode_num);
	}
	// Padding
	for (u32 inode_num = NUM_INODES + 1; inode_num <= NUM_BLOCKS * 8; inode_num++) {
    	set_bit(bitmap, inode_num);
	}
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_size = strlen(content);
